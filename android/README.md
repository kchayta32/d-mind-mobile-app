...
- `DMIND_ADMIN_TOKEN` สำหรับป้องกัน `/notifications/send`
- `SUPABASE_SERVICE_ROLE_KEY` สำหรับ backend gateway, report storage, media upload และ token registry
- `DMIND_TOKEN_STORE_PATH` สำหรับ local persistent FCM token registry fallback
- `DMIND_RELEASE_STORE_FILE`, `DMIND_RELEASE_KEY_ALIAS`, `KEYSTORE_PASSWORD` และ `KEY_PASSWORD` สำหรับ release signing
...
| GET | `/weather` | proxy ข้อมูล TMD แบบ live เมื่อมี `TMD_API_TOKEN` |
| POST | `/damage-assessment` | ปิด mock แล้ว คืน not-configured จนกว่าจะตั้ง production model endpoint |
| POST | `/chat` | ปิด placeholder แล้ว คืน not-configured จนกว่าจะ wire production chat gateway |
| POST | `/media/incident-images` | อัปโหลดรูป incident เข้า Supabase Storage ผ่าน backend gateway |
| POST | `/fcm/register` | รับ FCM token จากมือถือ |
| POST | `/notifications/send` | ส่ง push notification ผ่าน FCM HTTP v1 ต้องมี `Authorization: Bearer <DMIND_ADMIN_TOKEN>` |
...
- `/chat` และ `/damage-assessment` ไม่คืนผล mock แล้ว แต่ยังต้องต่อ production AI gateway/model endpoint จริง
- FCM token registry มี local persistent fallback และ sync Supabase table `device_push_tokens` เมื่อ config พร้อม
- `POST /notifications/send` มี admin bearer token แล้ว ต้องตั้ง `DMIND_ADMIN_TOKEN` ก่อนใช้ production
